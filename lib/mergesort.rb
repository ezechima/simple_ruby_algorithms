...
module MergeSort_C 
    ...
    def split(array)
        len = array.length
        len == 1 || len == 2 ? array : [array[0..len/2],array[len/2+1..len-1]]
    end
    ...
    def merge(array_a, array_b)
        ...
        while(array_a.length>0 && array_b.length > 0) do
            merged_array.push(array_a[0] <= array_b[0] ? array_a.slice!(0) : array_b.slice!(0))
        end
    ...
    def merge_sort (unsorted_array)
        ...
        split_b = splitarray[1]

        merge(split_a.length >2 ? merge_sort(split_a) : small_sort(split_a), split_b.length > 2 ? merge_sort(split_b) : small_sort(split_b))
